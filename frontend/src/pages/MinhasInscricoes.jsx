      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-600 mt-1">
            Gerencie suas inscrições na Copa PassaBola
          </p>
        </div>
        <Link
          to="/copa/inscricao/nova"
      ...
      {inscricoes.length === 0 ? (
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-12 text-center">
          ...
          <p className="text-gray-600 mb-6">
            Comece criando sua primeira inscrição para a Copa PassaBola
          </p>
          <Link
            to="/copa/inscricao/nova"
            className="inline-flex items-center gap-2 px-4 py-2 bg-green-600 text-white rounded-lg font-medium hover:bg-green-700 transition"
                ...
                <tr key={inscricao.id} className="hover:bg-gray-50">
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm font-medium text-gray-900">
                      {inscricao.time?.nome || "Sem nome"}
                    </div>
                    <div className="text-sm text-gray-500">
                      {inscricao.atletas?.length || 0} atletas
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <div className="flex items-center justify-end gap-2">
                      <Link
                        to={`/copa/inscricao/${inscricao.id}`}
                      ...
                      {inscricao.status === "pendente" && (
                        <>
                          ...
                          <button
                            onClick={() =>
                              handleExcluir(inscricao.id, inscricao.time?.nome)
                            }
